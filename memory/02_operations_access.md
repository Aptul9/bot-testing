...
## Connettivita' verso collaudo

- Gli endpoint `api-coll.museiitaliani.it` sono pubblici. Non richiedono VPN per essere raggiunti dal client dei BOT.
- L'accesso alla console WAF FortiGate non e' previsto da questo scope. Il SOC e' il referente per estrazione log e verifiche whitelist.
...
- La copia reale `secrets.md` e' in `.gitignore`. Chi subentra in un nuovo ambiente chiede il file all'owner e lo salva in `memory/secrets.md` localmente.
...
- Build e run dei BOT avviene via `docker build` + `docker run`. Lo sviluppo iterativo locale puo' usare venv con Python 3.11; la release finale gira con Python 3.12 nell'immagine.
...
| Ruolo | Referente | Responsabilita' |
|---|---|---|
